...
export function CategoryNav() {
    return (
        <div className="py-4 bg-white border-b border-gray-100">
            <h2 className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Shop By Category</h2>
            <div className="flex gap-4 overflow-x-auto px-4 pb-2 snap-x hide-scrollbar">
                {categories.map((cat) => (
                    <Link key={cat.id} href={`/menu?category=${cat.id}`} className="snap-start flex flex-col items-center flex-none gap-2 group">
                        ...
                        <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wide group-hover:text-orange-600 transition-colors">{cat.name}</span>
                    </Link>
                ))}
            </div>
        </div>
